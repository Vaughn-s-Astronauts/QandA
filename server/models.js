...
exports.addAllQuestions = (load1, load2) => {
  ...
  .then(() => {
    ...
    db.query(`CREATE TABLE questions (question_id integer PRIMARY KEY, product_id integer REFERENCES products (product_id), body TEXT, date_written VARCHAR(255), asker_name VARCHAR(1023), asker_email VARCHAR(255), reported BOOLEAN, helpful integer);`)
  })

  .then(() => {
    console.log('table questions created.')
    return db.query(`INSERT INTO questions (question_id, product_id, body, date_written, asker_name, asker_email, reported, helpful) SELECT * FROM json_to_recordset('${JSON.stringify(load1)}') as x(id integer, product_id integer, body TEXT, date_written VARCHAR(255), asker_name VARCHAR(1023), asker_email VARCHAR(255), reported BOOLEAN, helpful integer);`)
  })

  .then((res) => {
    console.log(`${res.rowCount} records added to the questions table.`)
    return db.query(`INSERT INTO questions (question_id, product_id, body, date_written, asker_name, asker_email, reported, helpful) SELECT * FROM json_to_recordset('${JSON.stringify(load2)}') as x(id integer, product_id integer, body TEXT, date_written VARCHAR(255), asker_name VARCHAR(1023), asker_email VARCHAR(255), reported BOOLEAN, helpful integer);`)
  })
...
exports.addFirstAnswers = (arrObj) => {
  ...
  .then(() => {
    ...
    db.query(`CREATE TABLE answers (answer_id integer PRIMARY KEY, question_id integer REFERENCES questions (question_id), body TEXT, date_written VARCHAR(255), answerer_name VARCHAR(1023), answerer_email VARCHAR(255) DEFAULT null, reported BOOLEAN, helpful integer);`)
  })

  .then(() => {
    console.log('table answers created.')
    return db.query(`INSERT INTO answers (answer_id, question_id, body, date_written, answerer_name, answerer_email, reported, helpful) SELECT * FROM json_to_recordset('${JSON.stringify(arrObj)}') as x(id integer, question_id integer, body TEXT, date_written VARCHAR(255), answerer_name VARCHAR(1023), answerer_email VARCHAR(255), reported BOOLEAN, helpful integer);`)
  })
